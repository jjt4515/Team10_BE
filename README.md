...
- [👤 내 역할](#-내-역할)
    - [Backend](#-backend)
    - [Trouble Shooting 1](#-trouble-shooting-1)
    - [Trouble Shooting 2](#-trouble-shooting-2)
...
- **JWT 기반 인증 시스템 구축**
  - 기존 Refresh Token 방식의 보안 취약점 해결을 위해 **RTR (Refresh Token Rotation)** 기법과 **화이트/블랙리스트 기반 토큰 검증 시스템**을 도입
    → Refresh Token 재사용 공격 차단, 사용자 로그아웃 후 토큰 즉시 무효화 
...
- **CI/CD 파이프라인 구축**
  ...
  - **AWS ECS + ALB 기반 무중단 배포 환경 구축**
    → 재배포 시 요청 손실 0건, 트래픽 분산으로 인한 서비스 가용성 향상
...
JWT 화이트리스트와 블랙리스트를 관리하는 시스템을 구축하면서, 배포 환경과 로컬 환경에서 **서로 다른 요구사항이 발생**했습니다.
- 배포 환경: 고성능 처리를 위해 화이트/블랙리스트를 Redis에 저장 
- 로컬 환경: Redis 설정 없이 간편하게 개발 및 테스트할 수 있는 환경 필요  
  → Redis 사용 시, 모든 팀원이 로컬에 Redis를 설치해야 하는 번거로움 발생

이로 인해, 화이트/블랙리스트 저장 방식의 차이로 코드 중복과 유지보수의 어려움이 발생했습니다.
...
- **인터페이스 기반 설계 + 환경별 DI 구성**
  - 환경별 설정 파일 분리
     - 로컬: `application.yml`
     - 배포: `application-prod.yml`
  - 실행 환경에 맞게 Redis 또는 MySQL을 사용하도록 인터페이스 기반 설계
  - 구현체는 DI를 통해 환경 별 설정에 따라 자동 주입
    
  - 로컬 환경: MySQL 기반 저장소 + 만료 토큰 자동 삭제를 위한 스케줄러 도입 
  - 배포 환경: 고성능 처리를 위한 Redis 기반 저장소 사용
...
이는 Refresh Token 검증 시 해당 토큰으로부터 **MemberId를 O(1) 로 조회**하기 위함이었습니다.  
...
**Redis는 싱글 스레드**로 동작하기 때문에 **KEYS 명령어는 전체 키를 탐색하는 동안 서버를 Blocking**하며, 다른 요청을 처리하지 못하게 합니다. 이는 대규모 데이터 환경에서 큰 성능 저하를 유발합니다.  

#### ⭐ 해결 방법

- **SCAN 명령어를 사용**하여 Blocking 없이 작은 배치 단위로 키를 탐색
- (key, value)를 (RefreshToken + MemberId, MemberId)로 구성
 → 값 조회 없이 키만으로도 MemberId 식별 및 삭제 가능  
